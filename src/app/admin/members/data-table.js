...
export function DataTable({ columns, data }) {
  ...
  const handleUpadateChanges = async () => {
    const confirmation = confirm('Are you sure you want to Update details')

    if (!confirmation) return

    try {
      const res = await API.put(`members/${editableData._id}`, editableData)
      if (res.status === 200) {
        toast({
      ...
      {selectedRow && (
        <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
          ...
          <SheetContent className="dark:bg-secondary-dark dark:text-primary-base overflow-auto">
            ...
            <SheetFooter>
              {isEditMode ? (
                <Button type="button" onClick={handleUpadateChanges}>
                  Save Changes
                </Button>
              ) : (
              ...
              <Button
                variant="destructive"
                type="button"
                onClick={() => deleteMember(editableData._id)}
              >
                Delete
              </Button>
            </SheetFooter>
